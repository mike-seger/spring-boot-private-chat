...
public interface ChatChannelRepository extends CrudRepository<ChatChannel, String> {
	...
	public List<ChatChannel> findExistingChannel(@Param("userOneId") long userOneId, @Param("userTwoId") long userTwoId);

	@Query("SELECT uuid FROM ChatChannel c WHERE c.userOne.id IN (:userIdOne, :userIdTwo) AND c.userTwo.id IN (:userIdOne, :userIdTwo)")
